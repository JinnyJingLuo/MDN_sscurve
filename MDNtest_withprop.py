...
X_train,  y_train, transformer_strain, transformer_gs, transformer_strainrate,  scaler_density,scaler_YP, scaler_ShearModulus, scaler_LatticeConsts, scaler_PoissonRatios  = normalizedata(data,test_size)
...
den_true =np.exp( scaler_density.inverse_transform( y_train.to_numpy().reshape(-1,1)))
...
plt.errorbar( y_train.to_numpy(),  np.sum(pi_data * mu_data,axis=1), np.sqrt(np.sum(pi_data**2 * sigma_data**2,axis=1)), fmt='o', capsize=5, ecolor='red', linestyle=' ', color='blue', label='Data with Error')
...
#%%
list_save = [network, transformer_strain, transformer_gs, transformer_strainrate,  scaler_density,scaler_YP,scaler_ShearModulus, scaler_LatticeConsts, scaler_PoissonRatios ]
